...
EXCLUDED_RIDES = {
    'RIDE_20260416_102808',
    'RIDE_20260331_151943',
    'RIDE_20260330_155917',
}

# Noise floor: readings below this are considered dry/pre-sweat and excluded from the mean
ACTIVE_SWEAT_FLOOR = 0.05  # L/hr
...
def fetch_mean_active_sweat(client):
    ...
    # Only average over readings where sweating is actively occurring
    active = df[df['sweat_rate_l_hr'] > ACTIVE_SWEAT_FLOOR]
    mean_df = active.groupby('ride_id')['sweat_rate_l_hr'].mean().reset_index()
    mean_df.rename(columns={'sweat_rate_l_hr': 'mean_active_sweat_rate_l_hr'}, inplace=True)
    return mean_df

def print_ride_table(merged_df):
    df = merged_df.copy()
    df['sweat_perception_score'] = pd.to_numeric(df['sweat_perception_score'], errors='coerce')
    df = df.sort_values('mean_active_sweat_rate_l_hr', ascending=False)
    print("\n  Rides sorted by mean active sweat rate:")
    print(f"  {'Ride ID':<32} {'Mean Active (L/hr)':>19} {'Survey Score':>13}")
    print("  " + "-"*66)
    for _, row in df.iterrows():
        print(f"  {row['ride_id']:<32} {row['mean_active_sweat_rate_l_hr']:>19.3f} {int(row['sweat_perception_score']):>13}")

def plot_correlation(merged_df, output_dir):
    print("Generating Mean Active Sweat Rate vs. Survey Score scatter plot...")

    plt.figure(figsize=(10, 6))

    x = merged_df['mean_active_sweat_rate_l_hr'].fillna(0)
    ...
    plt.title("Mean Active Sweat Rate vs. Post-Ride Perceived Sweat Score", fontsize=14, fontweight='bold')
    plt.xlabel(f"Mean Sweat Rate During Active Sweating (L/hr)  [floor: >{ACTIVE_SWEAT_FLOOR} L/hr]", fontsize=11)
    ...
    output_path = os.path.join(output_dir, '03.6_mean_active_sweat_rate_survey.png')
...
def analyse_threshold(merged_df):
    print("\n" + "="*60)
    print("  THRESHOLD CALIBRATION (Mean Active Sweat Rate)")
    print("="*60)

    df = merged_df.copy()
    df['sweat_perception_score'] = pd.to_numeric(df['sweat_perception_score'], errors='coerce')
    df['mean_active_sweat_rate_l_hr'] = pd.to_numeric(df['mean_active_sweat_rate_l_hr'], errors='coerce')
    df = df.dropna(subset=['sweat_perception_score', 'mean_active_sweat_rate_l_hr'])

    labels = {1: 'Completely Dry', 2: 'Lightly Sweating',
              3: 'Moderately Sweaty', 4: 'Very Sweaty', 5: 'Max Effort (Dripping)'}

    print(f"\n  Mean active sweat rate per subjective rating (floor >{ACTIVE_SWEAT_FLOOR} L/hr):")
    print(f"  {'Rating':<10} {'Label':<28} {'Mean Active (L/hr)':<20} {'N Rides'}")
    print("  " + "-"*64)

    band_means = {}
    for rating in sorted(df['sweat_perception_score'].unique()):
        subset = df[df['sweat_perception_score'] == rating]['mean_active_sweat_rate_l_hr']
        mean_val = subset.mean()
        band_means[rating] = mean_val
        label = labels.get(int(rating), 'Unknown')
        print(f"  {int(rating):<10} {label:<28} {mean_val:<20.3f} {len(subset)}")
    ...
    if 2 in band_means and 3 in band_means:
        ...
        print(f"\n  Mean active rate at rating 2 (Lightly Sweating)  : {band_means[2]:.3f} L/hr")
        print(f"  Mean active rate at rating 3 (Moderately Sweaty) : {band_means[3]:.3f} L/hr")
...
def main():
    print("="*60)
    print(" STEP 3.6: MEAN ACTIVE SWEAT RATE vs. SURVEY SCORE")
    print("="*60)

    client = create_client(SUPABASE_URL, SUPABASE_KEY)

    surveys_df = fetch_surveys(client)
    telemetry_df = fetch_mean_active_sweat(client)
